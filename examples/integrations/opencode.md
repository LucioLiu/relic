...
2. 填写 SOUL.md 和 USER.md

## 第二步：植入锚点（关键步骤）

> 💡 以下为简化版锚点。完整版（含轻量模式、断开指引）见 `brain/PROTOCOL.md` 第九节。

在 OpenCode 的 **WORK_RULES.md** 或 **instructions 配置** 中加入：
...
步骤5：把 Relic MEMORY.md 中你不知道的新记忆，追加到你自己的记忆文件里
  ...
  "你自己的记忆文件"指 OpenCode 的 WORK_RULES.md 或 instructions 中的记忆段。如果不确定往哪写，问用户。
...
启动 OpenCode，问它：
> "你读了我的 Relic 吗？告诉我我的 AI 叫什么名字。"

## 注意事项

OpenCode（即 Atlas）的配置方式是通过 `opencode.json` 中的 `instructions` 字段，
或者项目根目录的 `WORK_RULES.md` 文件。
把锚点写在任一位置即可。
