...
| 🔴 只读 | ARCHIVE/, SESSIONS/ | 只能读，永远不能修改或删除 |
...
├── MEMORY.md                # 🟡 追加写 — 精炼记忆
├── PROTOCOL.md              # 躯壳接入协议（中文版）
├── PROTOCOL.en.md           # 躯壳接入协议（English）
├── .relic-version           # 版本号
...
接入 Relic 后，必须在躯壳自己的核心配置文件里植入锚点。完整锚点模板见 `brain/PROTOCOL.md` 第九节。
